...
class square():
	...
	def draw(self, game_display, eyes = False):
		...
		pygame.draw.rect(game_display, self.colour, (a*dd + 1, b*dd + 1, dd - 2, dd - 2))
		if eyes:
			c = dd // 2
			r = 3
			eye_1 = (a*dd + c - r, b*dd + 8)
			eye_2 = (a*dd + dd - r*2, b*dd + 8)
...
class snake():
	...
	# Add a Square to the Snake After Eating Each Apple
	def add_square(self):
		tail = self.body[-1]
	...
	def draw(self, game_display):
		for num, square_ in enumerate(self.body):
			if num == 0:
				square_.draw(game_display, True)
			else:
	...
	while True:
		...
		for b in range(len(sn.body)):
			if sn.body[b].position in list(map(lambda l: l.position, sn.body[b + 1:])) or sn.body[b].position in list(map(lambda w: w.position, walls)):
				message("You Lost!", "Your score was: {}".format(len(sn.body)))
				sn.reset((6, 6))
				break
